...
| `GATEFORGE-ADMIN-PORTAL-CONTROL-TOWER.md` | **Product / architecture source of truth.** Control Tower positioning, operational modules, ADM-xxx backlog (P0/P1/P2), Given/When/Then acceptance criteria, suggested SQLite data model, API outline, 5-phase roadmap, read-only v1 guardrails, n8n positioning | **Read second** — authoritative for product direction, scope, backlog |
| `GATEFORGE-ADMIN-PORTAL-IMPLEMENTATION.md` | **Canonical build plan / execution guide.** Phase-by-phase delivery against the Control Tower roadmap: repo structure, tech stack, task IDs, data-model implementation notes, API implementation, UI routes, SSE events, validation rules, testing, deployment, agent build instructions, open questions | **Read third** — how the portal will actually be built |
...
| `GATEFORGE-ADMIN-PORTAL-EXTENDED-FEATURES.md` | Specification for the 22 extended features (v1.5–v2.5) and Category G infra features, with priority matrix and nav structure | When you need the exact behaviour of a non-v1.0 feature |
| `README.md` (root) | GateForge architecture — hub-and-spoke, roles, communication | When you need to understand the parent system |

> **Positioning note (v0.3):** the Admin Portal is the **read-only operational Control Tower and trust layer** for the GateForge Agentic SDLC. It does not control agents in v1 — human interaction continues to route through Telegram → Architect. See `GATEFORGE-ADMIN-PORTAL-CONTROL-TOWER.md` §1 for the full framing, §2 for the operational modules (Mission Control, Agent Fleet, Notification Center, Dead-letter Queue, Blueprint Governance, Security & Secrets, Model & Provider Resilience, Healthcare Overlay, Setup & Installer, Audit & Management Reports, Future RBAC & Multi-project), and §9 for why n8n initially sits *outside* the core SDLC loop as an integration layer for intake / approvals / notifications / reporting.
