...
show_help() {
  ...
  echo -e "    7.  Write secrets to /opt/secrets/openclaw-vm1.env"
...
"
  ...
  print_summary_line "Secrets File:" "/opt/secrets/openclaw-vm1.env"
  print_summary_line "Systemd Service:" "openclaw-vm1"
  ...
  echo -e "    1. Start the service:  ${TEAL}sudo systemctl start openclaw-vm1${RESET}"
  echo -e "    2. Check status:       ${TEAL}sudo systemctl status openclaw-vm1${RESET}"
  echo -e "    3. View logs:          ${TEAL}journalctl -u openclaw-vm1 -f${RESET}"
